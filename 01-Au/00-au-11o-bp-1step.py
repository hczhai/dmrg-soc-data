...
driver = DMRGDriver(
    scratch="/central/scratch/hczhai/soc-01-%02d" % fidx, symm_type=SymmetryTypes.SGFCPX, n_threads=24,
    stack_mem=int(40E9), mpi=True
)

if driver.mpi.rank == driver.mpi.root:

    mol = gto.M(atom="Au 0 0 0", basis="ano@8s7p4d2f", verbose=4, spin=1, max_memory=100000)
    print('basis = ano@8s7p4d2f nelec = %d nao = %d' % (mol.nelectron, mol.nao))
    ...
    selected = b2mcscf.select_active_space(
        mol, mf.mo_coeff, mf.mo_occ, ao_labels=['Au-5d', 'Au-6d', 'Au-6s'], atom_order=[0]
    )
